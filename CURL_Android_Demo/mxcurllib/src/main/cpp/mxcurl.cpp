...
jmethodID findMethod(JNIEnv *env, const char *class_name, const char *method_name,
...
JNI_OnLoad(JavaVM *jvm, void *reserved) {
    ...
    cls = env->FindClass("com/bbbond/mxcurllib/Curl$WriteCallback");
    if (cls == NULL) {
        return JNI_ERR;
    }
    ...
    cls = env->FindClass("com/bbbond/mxcurllib/Curl$ReadCallback");
    if (cls == NULL) {
        return JNI_ERR;
    }
    ...
    const char *multipart = "com/bbbond/mxcurllib/easy/MultiPart";
    ...
    MID_MultiPart_get_content_type = findMethod(env, multipart, "getContentType",
                                                "()Ljava/lang/String;");
...
Java_com_bbbond_mxcurllib_Curl_curlGlobalCleanupNative(
        JNIEnv *env,
        jclass type
) {
    curl_global_cleanup();
...
Java_com_bbbond_mxcurllib_Curl_curlEasyInitNative(
        JNIEnv *env,
        jobject instance
) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasyCleanupNative(
        JNIEnv *env,
        jobject instance,
        jlong handle
) {
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasySetoptLongNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jint opt,
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasySetoptFunctionNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jint opt,
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasySetoptObjectPointNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jint opt,
    ...
    CURL *curl = holder->getCurl();
    jstring value_ref;
    str = env->GetStringUTFChars(value_, 0);
    if (str == 0) {
        return 0;
    }
    ...
    switch (opt) {
        case CURLOPT_POSTFIELDS:
            ...
            value_ref = (jstring) env->NewGlobalRef(value_);
            holder->addStringGlobalRefs(value_ref, str);
            break;
        default:
            // free
            env->ReleaseStringUTFChars(value_, str);
    }
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasySetoptObjectPointBytesNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jint opt,
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasySetoptObjectPointArrayNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jint opt,
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_setFormdataNative(
        JNIEnv *env,
        jobject instance,
        jlong handle,
        jobjectArray multi_array
) {
    Holder *holder = (Holder *) handle;
    if (holder == NULL) {
        return 0;
    ...
    if (multi_array != NULL) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            jstring content_type = (jstring) env->CallObjectMethod(part,
                                                                   MID_MultiPart_get_content_type);
            jbyteArray content = (jbyteArray) env->CallObjectMethod(part,
                                                                    MID_MultiPart_get_content);
            jbyte *bytes = env->GetByteArrayElements(content, 0);
            int content_length = env->GetArrayLength(content);

            holder->addByteArrayGlobalRefs(env->NewGlobalRef(content),
                                           (const char *) bytes); // release after perform
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_bbbond_mxcurllib_Curl_curlEasyPerformNavite(
        JNIEnv *env,
        jobject instance,
        jlong handle
) {
