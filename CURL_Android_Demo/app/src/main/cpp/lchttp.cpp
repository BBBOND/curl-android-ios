...
lchttp_error_t *lchttp_error_init() {
    lchttp_error_t *error = (lchttp_error_t *) malloc(sizeof(lchttp_error_t));

    if (error) {
        error->code = (CURLcode) 0;
        error->message = NULL;
    }
    return error;
}
...
lchttp_response_t *lchttp_response_init() {
    lchttp_response_t *response = (lchttp_response_t *) malloc(sizeof(lchttp_response_t));
...
size_t lchttp_header_callback(char *data, size_t size, size_t nitems, void *userp) {
    ...
    response->header = (char *) realloc(response->header, new_size + 1);
...
static size_t lchttp_response_callback(void *data, size_t size, size_t nmemb, void *userp) {
    ...
    response->text = (char *) realloc(response->text, new_size + 1);
